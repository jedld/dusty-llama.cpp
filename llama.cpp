...
static std::vector<llama_vocab::id> llama_tokenize_internal(const llama_vocab & vocab, std::string raw_text, bool bos, bool special) {
    ...
    while (delim_start < raw_text.size()) {
        size_t delim_end = 0;
        ...
        for (const auto & mit : special_token_to_id) {
            ...
            if (end <= raw_text.size() && raw_text.compare(delim_start, delimiter.size(), delimiter) == 0) {
        ...
        if (token_id != -1) {
            if (last_delim_end < delim_start) {
                tokenizer.tokenize(raw_text.c_str() + last_delim_end, delim_start - last_delim_end, output);
            }
            output.push_back(token_id);
    ...
    if (last_delim_end < raw_text.size()) {
        tokenizer.tokenize(raw_text.c_str() + last_delim_end, text.size() - last_delim_end, output);
    }
